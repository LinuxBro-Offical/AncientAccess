...
app.post('/cultural-attractions', upload.array('files'), async (req, res) => {
    try {
      // console.log(req.files)
     
      const { title ,description, ticketPrice } = req.body;
      let results = findFiles(req.files)
...
app.post('/tickets', async (req, res) => {
    ...
    try{
      ...
      // Loyality points adding on ticket purchace 
...
// Calculate the sum of finalAmount of all tickets
app.get('/api/tickets/totalAmount', async (req, res) => {
  try {
    ...
    res.json({ totalAmount: totalAmount[0].sum });
  } catch (error) {
    res.status(500).json({ error: 'An error occurred' });
  }
});

app.get('/api/tickets/totalAmount/day', async (req, res) => {
  try {
    ...
    res.json({ totalAmount: totalAmount[0]?.sum || 0 });
  } catch (error) {
    res.status(500).json({ error: 'An error occurred' });
  }
});
...
// API route to filter tickets by user
app.get('/tickets/user/:userId', async (req, res) => {
  const userId = req.params.userId;
    try {
      const tickets = await Ticket.find({ user: userId }).populate('culturalAttraction').populate('selectedEvent');
      res.json(tickets);
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: 'Server error' });
...
function findFiles(files){
  const photoMimeTypes = ['image/jpeg', 'image/png', 'image/gif'];
  const videoMimeTypes = ['video/mp4', 'video/mpeg', 'video/quicktime', 'video/x-matroska'];
